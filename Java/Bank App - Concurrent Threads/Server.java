...
public class Server extends Thread {
    ...
    public boolean processTransactions(Transactions trans) {
        ...
        while ((!objNetwork.getClientConnectionStatus().equals("disconnected"))) {
            // Needed this check here to stop the infinite loiop when clients is disconnected, when buffer is empty, context switch with yield
    ...
    public void run() {
        ...
        System.out.println("\n Terminating Server thread with running time : " + (serverEndTime - serverStartTime) + " milliseconds");
        objNetwork.disconnect(objNetwork.getServerIP());   // After transactions processed & clients disconnect, disconnect server
